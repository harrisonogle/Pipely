...
**Files:**
- Modify: `tests/Pipe.Tests/PipeContinuationDispatcherTests.cs`
...
Append this test method at the end of the `PipeContinuationDispatcherTests` class in `tests/Pipe.Tests/PipeContinuationDispatcherTests.cs` (just before the closing brace):
...
/// through PipelyAwaiter.OnCompleted (capturing the consumer-thread EC at that
...
```bash
dotnet test tests/Pipe.Tests --nologo --filter MultiCycle_PerCycleEcCapture_AppliesCorrectEcEachCycle
```
...
- [ ] **Step 3: Replace `src/Pipely/PipelyAwaiter.cs` with the new EC-capture form**

Write to `src/Pipely/PipelyAwaiter.cs` (overwriting):
...
internal sealed class PipelyAwaiter<T> : IValueTaskSource<T>
{
    ...
    // benchmark project (`tests/Pipe.Benchmarks/PipeAdapter.cs`) after a run completes;
...
- [ ] **Step 4: Replace `src/Pipely/Pipe.cs` with the simplified form**

Write to `src/Pipely/Pipe.cs` (overwriting):
...
- `s_dispatchReadSetResult`, `s_dispatchReadSetException`, `s_dispatchFlushSetResult`, `s_dispatchFlushSetException` static delegates — REMOVED (replaced by `s_dispatch` and `s_invokeWithEc` in `PipelyAwaiter<T>`).
- `internal void DispatchVia(Action<object?>, object?)` helper — REMOVED.
- Awaiter ctor lines went from `new PipelyAwaiter<ReadResult>()` to `new PipelyAwaiter<ReadResult>(dispatcher)`.
...
- [ ] **Step 5: Edit `src/Pipely/Pipe.Reader.cs` — convert the three signal sites**

Apply three edits to `src/Pipely/Pipe.Reader.cs`:
...
- [ ] **Step 6: Edit `src/Pipely/Pipe.Writer.cs` — convert the three signal sites**

Apply three edits to `src/Pipely/Pipe.Writer.cs`:
...
If `tests/Pipe.Tests/PipelyAwaiterTests.cs` fails to compile because its `new PipelyAwaiter<int>()` no longer matches the new `(IContinuationDispatcher)` constructor, **fix that test file in this same task** by changing every `new PipelyAwaiter<int>()` to `new PipelyAwaiter<int>(ThreadPoolContinuationDispatcher.Instance)`. Add a comment if helpful: `// Test-only — uses the default singleton dispatcher.` `ThreadPoolContinuationDispatcher` is `internal`, so the test project must already have access; if not, an `[InternalsVisibleTo("Pipe.Tests")]` attribute may be needed in `src/Pipely/Pipely.csproj`. Check first whether `PipelyAwaiterTests.cs` references any field-level signature changes — read the file before editing.

- [ ] **Step 8: Verify `PipelyAwaiterTests.cs` compiles cleanly**

Read `tests/Pipe.Tests/PipelyAwaiterTests.cs`. Locate every `new PipelyAwaiter<int>()`. Replace each with `new PipelyAwaiter<int>(ThreadPoolContinuationDispatcher.Instance)` (the unit tests don't actually exercise dispatch, they only test the field/state machine — passing the singleton TP dispatcher is harmless).

If `ThreadPoolContinuationDispatcher` is internal and inaccessible, add an `InternalsVisibleTo` element to `src/Pipely/Pipely.csproj`:
...
(Only add this if the build fails on inaccessibility — the existing test project already accesses `PipelyAwaiter<T>` which is also internal, so the attribute may already exist or may not be needed.)
...
The existing test `CustomDispatcher_BadImpl_CapturingEC_IsDetectable` in `tests/Pipe.Tests/PipeContinuationDispatcherTests.cs` continues to pass under the new wiring, but its inline rationale comments still refer to `MRVTSC.RunInternal` as the mechanism that protects the consumer from the bad dispatcher's EC capture. That mechanism is no longer in play — under the new wiring, `PipelyAwaiter.OnCompleted` captures the consumer's EC on the consumer's thread *before* the bad dispatcher ever sees the work item; `s_invokeWithEc` applies that captured EC regardless of what EC the bad dispatcher captured.
...
Apply this edit to `tests/Pipe.Tests/PipeContinuationDispatcherTests.cs`:
...
```csharp
    ...
    /// capture wiring, the consumer's EC is captured by PipelyAwaiter.OnCompleted
...
```csharp
        ...
        // continuation: PipelyAwaiter.OnCompleted already captured the consumer's
...
```bash
git add src/Pipely/PipelyAwaiter.cs src/Pipely/Pipe.cs src/Pipely/Pipe.Reader.cs src/Pipely/Pipe.Writer.cs tests/Pipe.Tests/PipeContinuationDispatcherTests.cs tests/Pipe.Tests/PipelyAwaiterTests.cs
git commit -m "$(cat <<'EOF'
PipelyAwaiter: source-side EC capture (OpenTcp pattern)
...
source-side PipelyAwaiter<T>. Closes Mechanism A (consumer-captured
...
Implementation
- PipelyAwaiter<T>: 3 new fields (_realContinuation, _realState,
...
- s_dispatch + s_invokeWithEc + s_runContinuation static delegates
  on PipelyAwaiter<T>: s_dispatch (registered with _core.OnCompleted)
...
- Pipe.cs / Pipe.Reader.cs / Pipe.Writer.cs: 12 signal
...
Append this test method to the `PipeContinuationDispatcherTests` class:
...
/// PipelyAwaiter.OnCompleted captures _capturedEC = null and forwards (s_dispatch,
...
public async Task SuppressFlow_AtAwait_NoCapturedEC_BranchExercisedCleanly()
{
    ...
    using (ExecutionContext.SuppressFlow())
    {
        ...
        // FlowExecutionContext = 0, so PipelyAwaiter.OnCompleted captures
...
```bash
dotnet test tests/Pipe.Tests --nologo --filter SuppressFlow_AtAwait_NoCapturedEC_BranchExercisedCleanly
```
...
```bash
git add tests/Pipe.Tests/PipeContinuationDispatcherTests.cs
git commit -m "$(cat <<'EOF'
PipelyAwaiter tests: C.3 — FlowExecutionContext suppression branch
...
Append to `PipeContinuationDispatcherTests` — the helper class first (above the existing `// ---------- Tests ----------` header is a natural spot, but appending at the end is also fine; keep alphabetical / topical proximity to the other helper dispatchers if convenient):
...
/// new PipelyAwaiter.OnCompleted strips UseSchedulingContext from the flags forwarded
...
```bash
dotnet test tests/Pipe.Tests --nologo --filter SynchronizationContext_AtAwait_NotHonored_ContinuationOnDispatcherThread
```
...
git add tests/Pipe.Tests/PipeContinuationDispatcherTests.cs
git commit -m "$(cat <<'EOF'
PipelyAwaiter tests: D.1 — SynchronizationContext capture bypassed
...
Spec ref: §3.3 Pipe public-contract scheduler-bypass clause.
...
Append to `PipeContinuationDispatcherTests`:
...
```bash
dotnet test tests/Pipe.Tests --nologo --filter TaskScheduler_AtAwait_NotHonored_ContinuationOnDispatcherThread
```
...
git add tests/Pipe.Tests/PipeContinuationDispatcherTests.cs
git commit -m "$(cat <<'EOF'
PipelyAwaiter tests: D.2 — TaskScheduler capture bypassed
...
```bash
dotnet test tests/Pipe.Tests --nologo --filter ConfigureAwait_TrueAndFalse_BothRunOnDispatcherThread
```
...
git add tests/Pipe.Tests/PipeContinuationDispatcherTests.cs
git commit -m "$(cat <<'EOF'
PipelyAwaiter tests: D.3 — ConfigureAwait(true/false) parity
...
identical observable behavior on a Pipe await: both run the
...
This test operates **directly on a `PipelyAwaiter<int>`** rather than going through `Pipe`. `Pipe.Reader.ReadAsync` has a synchronous fast path (`HasReadableProgress` after `TryAcquire`+integrate) that delivers data without ever calling `OnCompleted` if the producer has already published — so a Pipe-level test of "producer-flushes-before-consumer-awaits" never exercises the rare race path. Operating directly on `PipelyAwaiter` lets us call `_core.SetResult(value)` first and then manually invoke `awaiter.OnCompleted(...)`, which forces `MRVTSC` to see a completed source and queue `s_dispatch` to TP — the actual race the spec calls out. `PipelyAwaiter<T>` is `internal` and `Pipe.Tests` already has `InternalsVisibleTo` access.
...
///
/// Operates directly on PipelyAwaiter to force the race deterministically (Pipe's
...
public async Task SetResultBeforeOnCompleted_DirectAwaiter_Race_StressN_AllResultsDelivered()
{
    ...
    for (int i = 0; i < iterations; i++)
    {
        ...
        var awaiter = new PipelyAwaiter<int>(dispatcher);
        ...
        // CONSUMER REGISTERS SECOND (manually). PipelyAwaiter.OnCompleted writes
...
```bash
grep -n 'using System.Threading.Tasks.Sources' tests/Pipe.Tests/PipeContinuationDispatcherTests.cs
```
...
```bash
dotnet test tests/Pipe.Tests --nologo --filter SetResultBeforeOnCompleted_DirectAwaiter_Race_StressN_AllResultsDelivered
```
...
git add tests/Pipe.Tests/PipeContinuationDispatcherTests.cs
git commit -m "$(cat <<'EOF'
PipelyAwaiter tests: E.1 — SetResult-fires-first race (direct-awaiter stress)

Pins the spec §4 publication ordering by operating directly on
PipelyAwaiter<int> rather than through Pipe (whose synchronous
...
1. Contract item #2's text changes from "MUST NOT capture EC" to a revised description that says EC handling is now performed by `PipelyAwaiter<T>` (source-side capture/apply); the dispatcher is purely a thread router.
...
4. The "Where to look in the code" table needs the stash-fields row revised (now `_realContinuation` / `_realState` / `_capturedEC`) and the static-dispatch-delegates row revised (now `s_dispatch` / `s_invokeWithEc` on `PipelyAwaiter.cs`).
...
| Tests (EC flow, isolation, scheduler bypass, race) | `tests/Pipe.Tests/PipeContinuationDispatcherTests.cs` |
```
...
- Contract item #2: text now states EC handling is performed by
  PipelyAwaiter<T> (source-side capture/apply); dispatcher is purely a
...
## Task 9: Update Pipe spec — §5 fields, §5 continuation dispatch, §6 scheduler-bypass clause, I16

Per spec §8 Spec references: the `Pipe` design spec (`docs/superpowers/specs/2026-04-25-spsc-pipe-tripleBuffer-design.md`) needs three updates:
...
private static readonly Action<object?> s_dispatchReadSetResult = static state => {
    var a = (PipelyAwaiter<ReadResult>)state!;
...
git commit -m "$(cat <<'EOF'
Pipe spec: revise §5 / §6 / I16 / R10 for source-side EC capture

Updates the Pipe design spec to match the source-side EC-capture
implementation:
...
- I16: rewrite from "MRVTSC.RunInternal applies the captured EC" to
  "PipelyAwaiter.OnCompleted captures EC; s_invokeWithEc applies via
...
- Slot path: dispatcher writes s_invokeWithEc + awaiter into the slot;
  worker thread invokes; PipelyAwaiter applies the source-side-captured
...
```bash
dotnet build Pipe.slnx -c Debug
```
...
```bash
dotnet test Pipe.slnx -c Debug --nologo
```
...
```bash
grep -n 'private static readonly Action<object?> s_dispatch \|private static readonly Action<object?> s_invokeWithEc \|private static readonly ContextCallback s_runContinuation' src/Pipely/PipelyAwaiter.cs
```
...
```bash
grep -n 'public Action<object?>? _realContinuation\|public object? _realState\|public ExecutionContext? _capturedEC' src/Pipely/PipelyAwaiter.cs
```
...
```bash
grep -n 'public PipelyAwaiter(IContinuationDispatcher' src/Pipely/PipelyAwaiter.cs
```
...
```bash
grep -nE 'flags & ~suppressed|UseSchedulingContext' src/Pipely/PipelyAwaiter.cs
```
...
```bash
dotnet run -c Release --project tests/Pipe.Benchmarks --nologo -- latency --count 10000 --size 256 --trials 1
```
...
<sha> PipelyAwaiter tests: D.3 — ConfigureAwait(true/false) parity
<sha> PipelyAwaiter tests: D.2 — TaskScheduler capture bypassed
