...
// Steady-state companion to SchedulerBenchmarks. Each Pipe is constructed once
// in [GlobalSetup] and reused across all BDN iterations — neither side calls
...
// five pipe × scheduler variants.
//
// Pair against SchedulerBenchmarks: per-row (per-iter alloc) - (this row's
// per-iter alloc) ≈ amortized per-Pipe construction cost for that variant.
[MemoryDiagnoser]
public class SteadyStateSchedulerBenchmarks
{
