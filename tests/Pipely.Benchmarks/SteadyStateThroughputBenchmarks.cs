...
// Steady-state companion to ThroughputBenchmarks. Constructs the Pipe in
// [GlobalSetup] and reuses it across all BDN iterations — neither side calls
// Complete() between iterations. Reported allocations therefore exclude
// per-Pipe construction (TripleBuffers, awaiters, reader/writer wrappers,
// first BufferSegment) and isolate the per-1 MiB-transfer steady-state cost.
//
// Pair with ThroughputBenchmarks: (ThroughputBenchmarks per-iter alloc) -
// (this benchmark's per-iter alloc) ≈ amortized per-Pipe construction cost.
[MemoryDiagnoser]
public class SteadyStateThroughputBenchmarks
{
